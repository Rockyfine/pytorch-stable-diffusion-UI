...
def _load_standard_checkpoint(input_file: str, device: str):
    checkpoint_path = Path(input_file)
    if checkpoint_path.suffix == ".safetensors":
        if load_safetensors_file is None:
            raise ImportError(
                "Loading .safetensors base model requires the 'safetensors' package. "
                "Install it with: pip install safetensors"
            )
        return load_safetensors_file(str(checkpoint_path), device=str(device))
...
def load_from_standard_weights(input_file: str, device: str) -> dict[str, torch.Tensor]:
    ...
    original_model = _load_standard_checkpoint(input_file, device)
